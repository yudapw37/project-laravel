...
{    
            ...
            if($token=='8fjv76qp9l')
            {
                ...
                $result= [
                    ...
                    'bukuInvoice' => $this->bukuOrderSales($code_buku, $kodeAdminTrx, $fm_date, $t_date, 1, "regular", 1),
                     ...
                     'stockBukuSolo'=>$this->stockBuku($code_buku, 'Gd_001'),
                     'stockBukuJakarta'=>$this->stockBuku($code_buku, 'Gd_002')
    ...
    public function getDetailTransaksiSales(Request $request) {
        $token = $request->token;
        $kodeAdminTrx = $request->kodeAdminTrx;
        ...
        $t_date = substr($to_date,0,10)." 23:59:59";
         try {
            if($token=='2nke76qp9l')
            {
                // SELECT od.code_order, od.code_barang, mb.judul_buku, od.nama_promo, od.jumlah, od.harga, od.harga_promo, od.diskon, 
                // t.typeTransaksi, o.lunas, t.created_at, t.updated_at
                // FROM `orderdetail`od join `order`o on od.code_order=o.id
                // join ms_barang mb on od.code_barang = mb.id
                // join transaksi t on o.id=t.code_order
                // where t.typeTransaksi = 'tempo'
                // and t.created_at > '2020-09-24'
                // and t.code_status = '10'
                // and t.kodeAdminTrx LIKE '%CH00X%'
                // ORDER BY t.created_at ASC
                $rslt = DB::table('orderdetail')
                                ->select('orderdetail.code_order','orderdetail.code_barang','ms_barang.judul_buku','orderdetail.nama_promo',
                                'orderdetail.jumlah','orderdetail.harga','orderdetail.harga_promo','orderdetail.diskon','transaksi.typeTransaksi as typeTransaksi',
                                'order.lunas','transaksi.created_at','transaksi.updated_at')
                                ->join ('order','orderdetail.code_order', '=','order.id')
                                ->join ('ms_barang','orderdetail.code_barang', '=','ms_barang.id')
	       	...
	       	if ($typeTransaksi != 'tempo') {
			$rslt->whereIn('transaksi.code_status', [9, 10]);
                //	$rslt->where('transaksi.code_status', '=', '9');
            	}
               $result = $rslt->get();
            }
            else
    ...
    public function stockBuku($code_buku, $code_gudang) {

        try {            
            $buku = DB::table('ms_barang_stock')
                    ->select('d', 'k')
                    ->where('code_barang','=',$code_buku)
                    ->where('code_gudang','=',$code_gudang)
                    ->first();
    ...
    public function softDeletePromo(Request $request) {
        $token = $request->token;
        $code_promo = $request->code_promo;
