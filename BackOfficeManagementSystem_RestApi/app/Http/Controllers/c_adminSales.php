        ...
        try {
            return    DB::table('order')
                ->select(
                    ...
                    'order.diskonKodeUnik as diskonKodeUnik',
    ...
    public function updateConfirmationbyAdminSales(Request $request)
    {
        ...
        try {
            if ($token == '9g8w11thnv') {
                ...
                if ($gettransaksi->approve_keuangan == 0) {
                    $status = '4';
                    DB::table('transaksi')
                    ...
                    //     ->where('code_order', '=', $request->kodeOrderDetail)
                    //     ->first();
                    if ($gettransaksi->code_perusahaan == '1') {
                        $kodeAdminNew = 'WH%';
                    } else {
                        $kodeAdminNew = 'JKT%';
                    }
                    if ($statusKon->konf == '0') {
                        ...
                        $getadmin = DB::table('ms_admin')
                            ->select('kodeAdminTrx as kodeAdmin')
                        ...
                        foreach($getadmin as $ga){
                            if($ga->kodeAdmin!='-'){
                                $getTransaksiGudang = DB::table('transaksi')
                                ->where('kode_AWO', '=', $ga->kodeAdmin)
                                ->where('approve_gudang', '=', 0)->get();
                                $jml = count($getTransaksiGudang);
                                if($newjml < $jml){                                
                                    if($first == 0){$kodeAdminGudangNew = $ga->kodeAdmin;$newjml = $jml;$first = 1;}                                
                                }else{
                                    $newjml = $jml;
                                    $kodeAdminGudangNew = $ga->kodeAdmin;
                                } 
                            }
                        ...
                        //     ->orderBy('jumlah', 'asc')
                        //     ->first();
                        //   $kodeAdminGudangNew = $query->kodeAwo;
