...

{
    ...
    //     ->where('order.id','=',$idOrder)
        ...
        'order.kecamatan as kecamatan','order.kab_kota as kab_kota','order.propinsi as propinsi','order.pre_order as preorder', 'ms_customer.telephone as nomor','ms_customer.nama as nama','order.expedisi as ekspedisi','order.biayaExpedisi as biayaExpedisi','order.image as gambar','ms_status_trx.status as status','ms_status_trx.id as statusId','transaksi.approve_sales as approveSales','transaksi.approve_keuangan as approveKeuangan','transaksi.approve_sales2 as approveSales2','transaksi.approve_gudang as approveGudang','transaksi.keterangan as keteranganHold','order.created_at as tanggal')
            ...
            if(@$namaStaf){
                ...
                foreach($val_Sales as $val){
                    ...
                    ->select(DB::raw("(SELECT nama FROM ms_admin WHERE ms_admin.kodeAdminTrx = transaksi.kodeAdminTrx) as namaSales"),
                            ...
                            'transaksi.id as id', 'order.id as codeOrder','ms_customer.nama as nama','transaksi.typeTransaksi as typeTransaksi','ms_status_trx.status as status', 'order.nama_pengirim as namaPengirim','order.telephone_pengirim as nomorPengirim','order.nama_penerima as namaPenerima','order.telephone_penerima as telephonePenerima','order.alamat as alamat','order.expedisi as expedisi','transaksi.no_resi as noResiTable','transaksi.created_at as tanggal')
